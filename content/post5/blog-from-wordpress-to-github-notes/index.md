...
而是在使用 Claude Code CLI 过程中人自动懒惰的根本不想去看它生成的代码, 都懒到没编辑器打开过代码, 全在命令行下对话, 所以做完之后仍然是零编程经验,
学习到的知识也是零, 也没有经历过任何的思考, 只是每一步问 `process?` 的时候回签 `yes`. 但是经过人的一番调教之后, AI 是变得更聪明了,
然而使用 AI 的人却变得更愚蠢了. Claude Code CLI 会在碰到编译错误时自行分析, 解决; 也会当前系统只有 JDK 25 的情况下, 根据插件要求主动下载
JDK 17 解决了构建问题, 
...
也不会写作过程中提示该页无法响应了. 在线的话, 也能直接到 GitHub 上编辑文件, 提交, 生成静态页面发布是自动化的.